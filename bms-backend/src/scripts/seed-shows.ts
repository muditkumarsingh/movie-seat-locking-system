...
import { TheaterModel } from "../modules/threater/theater.model";
...
export const seedShow = async () => {
  const movieIds = ["69baebdfe98495004ae71bc4", "69baebdfe98495004ae71bc7"];
  const movies = await MovieModel.find({ _id: { $in: movieIds } });
  const theatres = await TheaterModel.find({ state: "Uttar Pradesh" });
